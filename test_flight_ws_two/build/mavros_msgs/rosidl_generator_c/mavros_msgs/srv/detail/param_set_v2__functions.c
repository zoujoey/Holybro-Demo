  ...
  mavros_msgs__srv__ParamSetV2_Request__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    mavros_msgs__srv__ParamSetV2_Request * data =
      (mavros_msgs__srv__ParamSetV2_Request *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!mavros_msgs__srv__ParamSetV2_Request__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          mavros_msgs__srv__ParamSetV2_Request__fini(&data[i]);
        }
        free(data);
        return false;
      }
  ...
  mavros_msgs__srv__ParamSetV2_Response__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    mavros_msgs__srv__ParamSetV2_Response * data =
      (mavros_msgs__srv__ParamSetV2_Response *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!mavros_msgs__srv__ParamSetV2_Response__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          mavros_msgs__srv__ParamSetV2_Response__fini(&data[i]);
        }
        free(data);
        return false;
      }
