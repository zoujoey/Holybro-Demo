...
class OffboardControl(Node):
    ...
    def timer_callback(self):
        ...
        self.publish_trajectory_setpoint((2*cos(2*pi*(t/100)),2*sin(2*pi*(t/100)),-2.0,0.0))
    ...
    def setpoint_reached(self):
        return ((self.pose.z>(self.setpoint_current.z-0.1) and self.pose.z<(self.setpoint_current.z+0.1)) and
                ((self.pose.x>(self.setpoint_current.x-0.1) and self.pose.x<(self.setpoint_current.x+0.1)) and
                (self.pose.y>(self.setpoint_current.y-0.1) and self.pose.y<(self.setpoint_current.y+0.1))))
    ...
    def publish_trajectory_setpoint(self, pose:tuple):
        ...
        self.get_logger().info(str(msg.z) +" " +str(self.setpoint_reached())+" "+str(self.pose.x)+" "+str(self.pose.y)+" "+str(self.pose.z)+" "+str(self.offboard_setpoint_counter_))
