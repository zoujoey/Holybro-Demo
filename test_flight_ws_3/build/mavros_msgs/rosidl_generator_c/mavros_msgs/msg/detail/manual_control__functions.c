  ...
  mavros_msgs__msg__ManualControl__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    mavros_msgs__msg__ManualControl * data =
      (mavros_msgs__msg__ManualControl *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!mavros_msgs__msg__ManualControl__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          mavros_msgs__msg__ManualControl__fini(&data[i]);
        }
        free(data);
        return false;
      }
