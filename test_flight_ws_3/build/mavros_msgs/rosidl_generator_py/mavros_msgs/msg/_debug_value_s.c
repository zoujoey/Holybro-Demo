...
bool mavros_msgs__msg__debug_value__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // array_id
    PyObject * field = PyObject_GetAttrString(_pymsg, "array_id");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->array_id = (int32_t)PyLong_AsLong(field);
    Py_DECREF(field);
  }
  {  // name
...
PyObject * mavros_msgs__msg__debug_value__convert_to_py(void * raw_ros_message)
{
  ...
  {  // array_id
    PyObject * field = NULL;
    field = PyLong_FromLong(ros_message->array_id);
    {
      int rc = PyObject_SetAttrString(_pymessage, "array_id", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
